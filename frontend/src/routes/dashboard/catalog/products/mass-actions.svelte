<script lang="ts">
	...
	let { checked, products }: Props = $props();
	...
	<Dialog.Content class="sm:max-w-lg">
			...
			<Dialog.Description class="text-center">
				...
				<ScrollArea orientation="vertical" class="mt-2 h-[52dvh] rounded-md bg-muted p-1.5">
					<ol class="flex min-w-0 flex-col items-start gap-1 text-left font-mono font-bold">
						{#each products.filter((ftr) => checked.includes(ftr.id)) as product, i}
							<li>{i + 1}. {product?.id} - {product?.name}</li>
						{/each}
					</ol>
				</ScrollArea>
			</Dialog.Description>
		...
		<Dialog.Footer>
			<div class="flex w-full justify-center gap-4">
				<form action="?/massDeleteProducts" method="POST" use:enhance={handleSubmit}>
